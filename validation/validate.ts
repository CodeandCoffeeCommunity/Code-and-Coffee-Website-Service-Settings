...
for(const settingsFile of readdirSync(join(__dirname,'../settings'))){
  ...
  if(!existsSync(join(__dirname,'/settings',settingsFile))){
    throw `Validation file missing for "${settingsFile}"`
  }
  const schema = JSON.parse(readFileSync(join(__dirname,'/settings',settingsFile),'utf-8'));
